let App = function() {
    ...
    let categoryScroll = {
        scrollCat: function() {
            let sidebarWrapper = document.querySelectorAll('.sidebar-wrapper li.active')[0];
                        ...
                        if (document.querySelector('.main-container').classList.contains('sidebar-closed')) {
                            if (e === 'mouseenter') {
                                ...
                                getMenuList.forEach(element => {
                                    ...
                                    let submenuShowEle = element.querySelector('.collapse.submenu.show');

                                    if (submenuShowEle) {
                                        submenuShowEle.classList.remove('show');
                                    }

                                    let submenuExpandedToggleEle = element.querySelector('.dropdown-toggle[aria-expanded="true"]');
    ...
    let inBuiltfunctionality = {
        ...
        bsTooltip: function() {
            ...
            for (let index = 0; index < bsTooltip.length; index++) {
                let tooltip = new bootstrap.Tooltip(bsTooltip[index])
            }
        },
        bsPopover: function() {
            let bsPopover = document.querySelectorAll('.bs-popover')
            for (let index = 0; index < bsPopover.length; index++) {
                let popover = new bootstrap.Popover(bsPopover[index])
            }
        },
